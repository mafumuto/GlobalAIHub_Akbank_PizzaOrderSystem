...
def display_goodbye():
    print("Pizza sipaiş sistemini kullandığınız için teşekkür ederiz")
    ...
    def extra_choice(cls):
        while True:
            try:
                ...
                if extra_choice == 0:
                    break
                elif extra_choice in cls.extra_choices:
...
den = PizzaFactory(Classic())
de = [Onion(), Meat(), Corn()]
den(*de)
print(den.get_cost())
print(den.get_description())

if __name__ == "__main__":
    display_header()

    p = Choice.pizza_choice()
    b = Choice.extra_choice()
    p(*b)
    print(p.get_cost())
